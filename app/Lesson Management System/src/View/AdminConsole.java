...
public class AdminConsole extends JFrame implements Runnable {
	...
	private JButton addButton, viewButton, manageLocationButton, manageAccountsButton, viewOfferingsButton,
			logoutButton;
	...
	private void handleAddLocation() {
		...
		if (typeOfSpace != null) {
			...
			displayMessage(
					result != null ? "Location added successfully." : "Location already exists or failed to add.");
		} else {
	...
	private void handleManageLocation() {
		...
		if (locations.isEmpty()) {
			displayMessage("No locations available to manage.");
			return;
		}

		// Convert the list of locations to an array of location names for the dropdown
		String[] locationNames = locations.stream().map(Location::getName).toArray(String[]::new);
		JComboBox<String> locationDropdown = new JComboBox<>(locationNames);

		// Show a dialog with the dropdown for location selection
		int result = JOptionPane.showConfirmDialog(this, locationDropdown, "Select a Location to Manage",
				JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

		if (result == JOptionPane.OK_OPTION) {
			String selectedLocationName = (String) locationDropdown.getSelectedItem();

			// Find the selected location object based on the name
			Location selectedLocation = locations.stream().filter(loc -> loc.getName().equals(selectedLocationName))
					.findFirst().orElse(null);

			if (selectedLocation != null) {
				manageSelectedLocation(selectedLocation);
			} else {
				displayMessage("Selected location not found.");
			}
		} else {
			displayMessage("Location selection canceled.");
		}
	}

	// New helper method to manage a selected location
	private void manageSelectedLocation(Location location) {
		String[] options = { "View Schedules", "Add Schedule", "Create Offering", "Back" };
		int choice = JOptionPane.showOptionDialog(this, "Manage options for " + location.getName(), "Manage Location",
	...
	private void handleAddSchedule(Location location) {
		...
		int dayOfWeek = Integer
				.parseInt(JOptionPane.showInputDialog(this, "Enter day of the week (1 for Monday, 7 for Sunday):"));
	...
	private void handleCreateOffering(Location location) {
		...
		// Prompt for integer-based times for easier handling
		String startTimeStr = JOptionPane.showInputDialog(this,
				"Enter offering start time in hours (e.g., 9 for 9 AM):");
		String endTimeStr = JOptionPane.showInputDialog(this, "Enter offering end time in hours (e.g., 13 for 1 PM):");

		try {
			int startTime = Integer.parseInt(startTimeStr);
			int endTime = Integer.parseInt(endTimeStr);

			// Continue only if times are valid
			if (startTime < 0 || startTime > 24 || endTime < 0 || endTime > 24 || startTime >= endTime) {
				displayMessage("Invalid start or end time. Please enter valid hours between 0 and 24.");
				return;
			}

			boolean isGroup = JOptionPane.showConfirmDialog(this, "Is this a group offering?", "Group Offering",
			...
			boolean success = offeringController.createOffering(location, selectedSchedule, startTime, endTime, isGroup,
					capacity, lessonType);
			displayMessage(success ? "Offering created successfully." : "Failed to create offering.");
		} catch (NumberFormatException e) {
			displayMessage("Invalid time format. Please enter numeric hours only.");
		} catch (IllegalArgumentException e) {
			displayMessage("Invalid lesson type or capacity. Please ensure all inputs are correct.");
		}
	}
	...
	private Schedule chooseSchedule(List<Schedule> schedules) {
		...
		String selectedOption = (String) JOptionPane.showInputDialog(this, "Select a schedule for the offering:",
				"Choose Schedule", JOptionPane.PLAIN_MESSAGE, null, scheduleOptions, scheduleOptions[0]);
	...
	private void handleManageAccounts() {
		...
		int choice = JOptionPane.showOptionDialog(this, "Account Management Options", "Manage Accounts",
				JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
	...
	private boolean isValidScheduleInput(String startDate, String endDate, int dayOfWeek, int startTime, int endTime) {
		return isValidDateFormat(startDate) && isValidDateFormat(endDate) && startDate.compareTo(endDate) < 0
				&& dayOfWeek >= 1 && dayOfWeek <= 7 && startTime >= 0 && startTime <= 24 && endTime >= 0
				&& endTime <= 24;
	}
